...
"#;
    ...
    // ═══════════════════════════════════════════════════════════
    // STORY-024 CA2: RunOptions.compact field
    // STORY-024 CA4: verbose = !compact en invoke_with_retry
    // ═══════════════════════════════════════════════════════════

    mod story024 {
        use super::*;

        // ── CA2: PipelineOptions (RunOptions) tiene campo compact ──

        /// CA2: RunOptions.compact existe y es false por defecto.
        #[test]
        fn run_options_compact_field_exists_and_defaults_false() {
            let opts = RunOptions::default();
            assert!(
                !opts.compact,
                "RunOptions.compact debe ser false por defecto"
            );
        }

        /// CA2: RunOptions.compact se puede establecer a true.
        #[test]
        fn run_options_compact_can_be_set_true() {
            let opts = RunOptions {
                compact: true,
                ..RunOptions::default()
            };
            assert!(opts.compact, "RunOptions.compact debe poder ser true");
        }

        /// CA2: RunOptions.compact se serializa/deserializa correctamente (es un campo normal).
        #[test]
        fn run_options_compact_field_is_accessible() {
        ...
        /// CA4: El valor de verbose es la negación de compact.
        /// compact=false → verbose=true; compact=true → verbose=false.
        #[test]
        fn verbose_is_not_compact() {
            // Sin --compact (default): verbose debe ser true
            let opts = RunOptions::default();
            assert!(!opts.compact, "default: compact=false");
            let verbose: bool = !opts.compact;
            assert!(verbose, "sin compact → verbose debe ser true");

            // Con --compact: verbose debe ser false
            let opts = RunOptions {
                compact: true,
                ..RunOptions::default()
            };
            assert!(opts.compact, "con --compact: compact=true");
            let verbose: bool = !opts.compact;
            assert!(!verbose, "con compact → verbose debe ser false");
        }

        /// CA4: La relación verbose = !compact se cumple para todas las combinaciones.
        #[test]
        fn verbose_not_compact_holds_for_both_values() {
            for compact_val in [false, true] {
                let opts = RunOptions {
                    compact: compact_val,
                    ..RunOptions::default()
                };
        ...
        /// CA4: process_story() recibe el parámetro compact y lo usa.
        /// Verifica que la firma de process_story incluye compact: bool.
        /// (El early-return path para Done/Bocked/Failed no depende del valor de compact.)
        #[tokio::test]
        async fn process_story_accepts_compact_param() {
            let tmp = tempfile::tempdir().unwrap();
            std::fs::create_dir_all(tmp.path().join(".regista/decisions")).unwrap();
            ...
            // Verificar que process_story acepta compact=true sin fallar
            // (Done → early return, no invoca agente)
            let story = story_fixture("STORY-001", Status::Done, None);
            let result = process_story(
                &story,
                tmp.path(),
            ...
            assert!(
                result.is_ok(),
                "process_story con compact=true debe funcionar (early return)"
            );

            // Verificar que process_story acepta compact=false sin fallar
            let story2 = story_fixture("STORY-002", Status::Failed, None);
            let result2 = process_story(
                &story2,
                tmp.path(),
                &cfg,
                &state,
            ...
            assert!(
                result2.is_ok(),
                "process_story con compact=false debe funcionar (early return)"
            );
        }
    }
}
